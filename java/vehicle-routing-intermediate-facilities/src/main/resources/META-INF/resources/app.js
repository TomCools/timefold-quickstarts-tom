...
const facilityMarkerByNameMap = new Map();
...
function getVisitMarker(visit) {
    ...
    if (!visit.location) {
        console.error('Visit missing location:', visit);
        return null;
    }
...
function facilityPopupContent(facility) {
    return `<h5>${facility.name}</h5>
<h6>Intermediate Facility</h6>`;
}

function getFacilityMarker(facility) {
    let marker = facilityMarkerByNameMap.get(facility.name);
    if (marker) {
        return marker;
    }
    const facilityIcon = L.divIcon({
        html: `<i class="fas fa-warehouse" style="color: #FF6B35; font-size: 24px; text-shadow: -1px -1px 0 #fff, 1px -1px 0 #fff, -1px 1px 0 #fff, 1px 1px 0 #fff;"></i>`,
        className: 'facility-icon',
        iconSize: [24, 24],
        iconAnchor: [12, 12]
    });
    marker = L.marker(facility.location, { icon: facilityIcon });
    marker.addTo(facilityGroup).bindPopup();
    facilityMarkerByNameMap.set(facility.name, marker);
    return marker;
}
...
function renderRoutes(solution) {
    ...
    // Facilities
    if (solution.facilities) {
        solution.facilities.forEach(function (facility) {
            const marker = getFacilityMarker(facility);
            marker.setPopupContent(facilityPopupContent(facility));
        });
    }
    // Route
    ...
    for (let vehicle of solution.vehicles) {
        ...
        vehicle.visits.forEach(visitOrId => {
            ...
            if (typeof visitOrId === 'object') {
                ...
                visit = visitByIdMap.get(visitOrId);
            }

            if (!visit) {
                console.error('Visit not found:', visitOrId);
                return;
            }
