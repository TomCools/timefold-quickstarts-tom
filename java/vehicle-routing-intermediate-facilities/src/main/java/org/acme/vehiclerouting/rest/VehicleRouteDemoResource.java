...
import org.acme.vehiclerouting.domain.Facilitiy;
...
public class VehicleRouteDemoResource {
    ...
    public enum DemoData {
        PHILADELPHIA(0, 55, 6, 5, LocalTime.of(7, 30),
                5, 10, 30, 40,
        ...
        FIRENZE(2, 77, 6, 3, LocalTime.of(7, 30),
                1, 2, 20, 40,
        ...
        DemoData(long seed, int visitCount, int vehicleCount, int facilityCount, LocalTime vehicleStartTime,
            ...
            if (facilityCount < 1) {
                throw new IllegalStateException(
                        "Number of facilityCount (%s) must be greater than zero.".formatted(vehicleCount));
            }
    ...
    public VehicleRoutePlan build(DemoData demoData) {
        ...
        List<Facilitiy> facilities = Stream.generate(facilityProvider)
                .limit(demoData.facilityCount)
                .collect(Collectors.toList());
